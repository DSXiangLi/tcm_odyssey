...
| 玩法要素 | 描述 |
|---------|------|
| **学习目标** | 掌握28种常见脉象+正常脉象的特征与临床意义 |
| **操作方式** | 根据古文描述选择脉位(浮沉迟数虚实平)和脉势(紧缓滑涩弦濡不明显) |
| **难度设计** | 初级(提示匹配)→中级(无提示)→高级(全选项) |
...
│                                                     │
│   脉位选择:                                          │
│   ○ 浮脉  ○ 沉脉  ○ 迟脉  ○ 数脉  ○ 虚脉  ○ 实脉  ○ 平脉(正常) │  ← SelectionButton组件(7选项)
│                                                     │
│   脉势选择:                                          │
│   ○ 紧脉  ○ 缓脉  ○ 滑脉  ○ 涩脉  ○ 弦脉  ○ 濡脉  ○ 不明显    │  ← SelectionButton组件(7选项)
│                                                     │
├─────────────────────────────────────────────────────┤
│                                                     │
│   [确认选择]                                         │  ← 确认按钮
...
| positionButtons | SelectionButton | 7 | 脉位选择(浮沉迟数虚实平) |
| tensionButtons | SelectionButton | 7 | 脉势选择(紧缓滑涩弦濡不明显) |
...
临床诊断中，并非所有病案都有明显异常脉象。部分病案的脉象可能表现正常（平脉），或脉势不明显。因此需要增加"正常"选项，避免玩家被迫选择异常脉象。

### 5.2 脉位新增选项

| 选项 | 名称 | 描述 | 临床意义 |
|-----|-----|------|---------|
| 平脉 | 正常脉位 | 脉位适中，不浮不沉 | 健康人或病情轻微 |
| 正确答案 | 'ping' | 当病案脉象描述无明显浮沉特征时 | 表示脉位正常 |

### 5.3 脉势新增选项

| 选项 | 名称 | 描述 | 临床意义 |
|-----|-----|------|---------|
| 不明显 | 正常脉势 | 脉势适中，无明显紧缓滑涩等特征 | 脉象柔和有力 |
| 正确答案 | 'normal' | 当病案脉势描述无明显异常时 | 表示脉势正常 |

### 5.4 正常脉象的数据结构

```typescript
// 病案脉象数据更新
interface CasePulseData {
  // 脉位：浮/沉/迟/数/虚/实/平(normal)
  position: 'fu' | 'chen' | 'chi' | 'shu' | 'xu' | 'shi' | 'ping';

  // 脉势：紧/缓/滑/涩/弦/濡/不明显(normal)
  tension: 'jin' | 'huan' | 'hua' | 'se' | 'xian' | 'ru' | 'normal';
...
// 正常脉象示例
const normalPulseCase: CasePulseData = {
  position: 'ping',      // 脉位正常
  tension: 'normal',     // 脉势正常
  description: '脉来从容和缓，不浮不沉，不快不慢，有力有根'
};
```
...
| 脉位选项(浮沉迟数虚实) | src/ui/PulseUI.ts | ✅ 完成(需增加平脉) |
| 脉势选项(紧缓滑涩弦濡) | src/ui/PulseUI.ts | ✅ 完成(需增加不明显) |
