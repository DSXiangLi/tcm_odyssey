...
## 本Session进展：NPC E2E测试策略设计 (2026-05-23) ✅ 设计完成
...
- `npc-dialog.spec.ts`: 837行（已存在，需扩展）
- `npc-feedback.spec.ts`: 383行（已存在，需扩展）
- `npc-tools.spec.ts`: 不存在（需新建）
- `npc-heartbeat.spec.ts`: 不存在（需新建）
- `utils/npc-test-helpers.ts`: 不存在（需新建）

### 下一步

使用 writing-plans skill 创建实现计划，然后执行实现。
