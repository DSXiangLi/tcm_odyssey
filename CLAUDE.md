...
> **详细内容**: [STATE.md](./STATE.md) - 已完成任务详情
...
### 校验原则
1. 每个设计文档和规划文档写完之后必须使用subagent模式调用`design-doc-reviwer` 技能对文档进行独立审核
...
```
每个新功能开发必须遵循以下流程：
1. 从master创建独立功能分支：`git checkout -b feature/xxx`
2. 在独立分支完成所有开发工作（设计+实现+测试）
3. 用户验收确认功能正常后
4. 合并到master分支：`git checkout master && git merge feature/xxx`
5. 推送到远程仓库
