...
## Phase 2.5: 病案集 HTML 嵌入 ✅ (2026-05-07)

**核心成果**:
- CasebookUI.tsx: 古籍风格病案展示（翻页动画）
- ClinicScene.ts: C键触发病案集快捷键
- CustomEvent桥接: React↔Phaser双向通信
- E2E测试: 18/18通过 (100%)
...
## Phase 2.5: 炮制 HTML 嵌入 ✅ (2026-05-07)

**核心成果**:
- PaozhiUI.tsx: 10种药材炮制流程
- GardenScene.ts: P键触发炮制快捷键
- 炮制方法匹配: 炒/炙/煅/蒸/煮等
- 辅料选择: 酒/醋/盐/蜜等
- E2E测试: 21/22通过 (95%)

**炮制药材 (10种)**:
当归(酒炙)、半夏(姜制)、地黄(熟地)、牡蛎(煅)、紫石英(煅)、干姜(炮)、黄芪(蜜炙)、大黄(酒制)、山药(麸炒)、神曲(炒)

**设计文档**: [炮制HTML嵌入设计](docs/superpowers/specs/phase2-5/2026-05-07-paozhi-embedding-design.md)

---

## Phase 2.5: HTML游戏尺寸统一 ✅ (2026-05-08)

**统一规范**:
所有HTML小游戏容器尺寸统一为 `min(1150px, 90vw) × min(680px, 85vh)`

**修复内容**:
| 游戏 | 原尺寸 | 新尺寸 | 状态 |
|------|--------|--------|------|
| 煎药 | 1150×680 (固定) | min(1150px, 90vw) × min(680px, 85vh) | ✅ |
| 诊断 | 1150×680 (固定) | min(1150px, 90vw) × min(680px, 85vh) | ✅ |
| 病案集 | 1150×680 (固定) | min(1150px, 90vw) × min(680px, 85vh) | ✅ |
| 炮制 | 100vw×100vh (全屏) | min(1150px, 90vw) × min(680px, 85vh) | ✅ |
| 背包 | 90vw×85vh | min(1150px, 90vw) × min(680px, 85vh) | ✅ |

**关闭按钮规范**:
- 位置: `position: absolute; top: 50px; right: 60px` (相对于内容容器)
- 尺寸: 44×44像素
...
*本文档由 Claude Code 维护，更新于 2026-05-11*