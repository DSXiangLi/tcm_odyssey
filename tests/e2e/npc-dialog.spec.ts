...
// ========================================
// Summary Test - Validates All Categories
// ========================================

test.describe('NPC Dialog - Integration Summary', () => {

  test('Full NPC dialog workflow integration', async ({ page, request }) => {
    // Integration test covering the full workflow
    // 1. Backend health
    const healthResponse = await request.get(`${HERMES_BACKEND_URL}/health`);
    ...
    // 4. Backend responds to dialog
    const chatResponse = await request.post(`${HERMES_BACKEND_URL}/v1/chat/stream`, {
      headers: { 'Content-Type': 'application/json' },
      data: {
        npc_id: 'qingmu',
        player_id: 'player_001',
