...
const CASES = window.MEDICAL_CASES;
...
// 印章组件（两字一组，2x2 或 单字旋转）
function Seal({ text, size = 'lg' }) {
  const sizeMap = {
    sm: { w: 56, fs: 22, br: 2 },
    md: { w: 80, fs: 28, br: 3 },
    lg: { w: 110, fs: 38, br: 4 },
...
// 左页：目录
function CategoryPage({ activeCat, setActiveCat, totalUnlocked, totalCount, casesData }) {
  return (
...
// 右页：病案网格
function CaseGridPage({ activeCat, casesData, onSelectCase }) {
  const cat = CASES.categories.find(c => c.id === activeCat);
...
// 详情·左：患者 + 病案概述 + 主诉 + 现病史 + 舌脉
function DetailLeft({ caseData, onBack }) {
  return (
...
// 详情·右：未解锁
function DetailRightLocked({ caseData, onStart }) {
  return (
    <div className="page-inner detail-scroll fade-in">
...
// 详情·右：已解锁
function DetailRightUnlocked({ caseData, onReplay }) {
  return (
    <div className="page-inner detail-scroll fade-in">
      ...
      <div className="score-block">
        <Seal text={caseData.stamp} size="lg" />
...
function App() {
  const [casesData] = useState(() => ({ ...CASES.cases }));
  ...
  const [pendingActiveCase, setPendingActiveCase] = useState(null);
  ...
  const handleStart = () => showToast('开 案 问 诊');
  const handleReplay = () => showToast('重 新 参 详');

  return (
    <>
      <SvgDefs />
      <div className="desk">
        <div className={`book ${flipDir ? 'flipping' : ''}`} data-screen-label={view === 'detail' ? '02 病案详情' : '01 病案目录'}>
          {/* 左页 */}
          <div className="page left">
            ...
            {view === 'list' ? (
              ...
              <DetailLeft caseData={activeCase} onBack={closeCase} />
            )}
          </div>
